...
The results are saved in a jsonl format for readability that you can check out in the repo (over **52k** entries !) , normally we should have created an item processing pipeline ,but getting exact origins and dates is very convoluted and varied per entry so the extraction of what we actually need will be done independently of this project which focuses mainly on getting the data.
...
## What is next?
- [x] Fashion an NLP processing pipeline.
- [x] Extract the relevant dates and points of origin.
- [ ] Make an API.
