...
The results are saved in a jsonl format for readability that you can check out in the repo (over **52k** entries !) , normally we should have created an item processing pipeline but getting exact origins or dates is very convoluted and varied per entry so the extraction of what we actually need will be done independently of this project.

But you can give it a go by going to items.py and pipelines.py!
